...
public class $FOLDER_CLASS_NAME$ implements IFoldingUpdater {
	...
	private class FoldingVisitor extends AbstractVisitor
	{
	    ...
	    private void makeAnnotation($AST_NODE$ n) {
		...
		// START_HERE
		//
		// Include visit(..) functions for various types of AST nodes
		// that are associated with folding.  These functions should create
		// ProjectionAnnotations corresponding to regions of text to be
		// folded.  Two versions of the function makeAnnotation(..) are
		// provided above to faciliate this.
		...
		IParseController parseController, ProjectionAnnotationModel annotationModel)
	{
		try {
			...
			$AST_NODE$ ast= ($AST_NODE$) parseController.getCurrentAst();
