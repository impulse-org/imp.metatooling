...
public class ImpPluginExtension extends 	//PluginParent implements IPluginExtension {
 PluginExtension {
	...
	public ImpPluginExtension() {
	}

	public ImpPluginExtension(IExtension extension) {
	...
	void load(Node node) {
		...
		try {
			setId(getNodeAttribute(node, "id"));
		} catch (CoreException e) {
			WizardPlugin.getInstance().writeErrorMsg("Couldn't set node id (but still continuing):  \n\t" + e.getMessage());
		}
